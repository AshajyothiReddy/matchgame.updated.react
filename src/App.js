...
class App extends Component {
  ...
  onChangeThumbnail = id => {
    const {appImagId, timer} = this.state
    if (id !== appImagId || timer === 0) {
      this.setState(prevState => ({isGameover: !prevState.isGameover}))
      clearInterval(this.timerId)
      console.log(id !== appImagId || timer === 0)
      console.log(timer === 0)
    } else {
      this.setState(prevState => ({score: prevState.score + 1}))
      const imagesListLength = imagesList.length
      const randomIndexNo = Math.ceil(Math.random() * imagesListLength)

      this.setState({
        initialImageUrl: imagesList[randomIndexNo].imageUrl,
        appImagId: imagesList[randomIndexNo].id,
      })
    }
  }
  ...
  render() {
    const {activeTabId, initialImageUrl, score, isGameover, timer} = this.state
    ...
    return (
      <div className="bg-container">
        ...
        {isGameover ? (
          ...
          <div className="image-tabs-thumb-container">
            <ul>
              <img src={initialImageUrl} className="app-image" alt="match" />
            </ul>
            ...
            <ul className="thumbnail-img-list">
              {filteredImages.map(eachItem => (
                <ThumbnailItem
                  ...
                  onClickThumb={this.onChangeThumbnail}
                />
              ))}
            </ul>
          </div>
